...
async def list_user_library_documents(
    ...
    statement = (
        ...
        .options(selectinload(models.db_models.Document.interactions))
...
async def delete_library_document(
    ...
    db: AsyncSession = Depends(get_db),
    vector_db: PgVectorService = Depends(get_vector_db_serv),   # type updated
    user: models.db_models.User = Depends(current_active_user),
):
    """
    Completely deletes a document from the library:
    1. Removes all document-interaction associations
    2. Deletes chunks from pgvector store
...
async def upload_document_to_library(
    ...
    try:
        ...
        if not processed_chunks:
            raise HTTPException(
                status_code=422,
                detail="Failed to process document. No chunks were generated.",
            )
...
@router.post(
    "/library/document/{document_id}/link-to/{interaction_id}",
    response_model=models.schemas.StatusResponse,
)
async def link_document_to_interaction(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail="An unexpected server error occurred while linking document.",
        )


@router.get(
    "/library/document/{document_id}/available-interactions",
    response_model=List[models.schemas.InteractionInfo],
)
async def get_available_interactions_for_document(
    ...
    """Lists all chat interactions NOT currently linked to the specified document."""
    ...
    available_interactions_statement = (
        select(models.db_models.ChatSession)
        .filter(
            models.db_models.ChatSession.owner_id == user.id,
